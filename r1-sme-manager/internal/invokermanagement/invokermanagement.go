...
import (
	...
	invokerapi "oransc.org/nonrtric/r1-sme-manager/internal/invokermanagementapi"
...
func (im *InvokerManager) PostOnboardedInvokers(ctx echo.Context) error {
	log.Info("Entering PostOnboardedInvokers")
	...
	if rspInvoker.StatusCode() != http.StatusCreated {
		...
		log.Infof("Error on PostOnboardedInvokersWithResponse %s", msg)
...
func (im *InvokerManager) DeleteOnboardedInvokersOnboardingId(ctx echo.Context, onboardingId string) error {
	log.Infof("Entering DeleteOnboardedInvokersOnboardingId onboardingId %s", onboardingId)
