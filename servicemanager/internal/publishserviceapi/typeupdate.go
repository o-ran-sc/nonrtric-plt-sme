...
func (sd *ServiceAPIDescription) createKongService(
	...
	log.Debugf("createKongService, serviceUri %s", kongServiceUri)

	apiId := *sd.ApiId
	kongServiceName := apiId + "_" + resourceName
	...
	log.Infof("createKongService, kongServiceNameUuid %s", kongServiceNameUuid)
	...
	if statusCode == http.StatusCreated {
		log.Infof("kong service %s created successfully", kongServiceNameUuid)
	...
	kongRouteUri, statusCode, err := sd.createRouteForService(kongControlPlaneURL, client, resource, routeName, uri, interfaceDescUuid, tags, foundRegEx)
	if err != nil {
		log.Errorf(err.Error())
		return kongRouteUri, statusCode, err
	}
